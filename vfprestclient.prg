...
Define Class Rest As Custom
	...
	Procedure addRequest(tcVerb, tcURL, tbSyncMode)
		With This
			If Pcount() <= 1
				MessageBox('Faltan par�metros: tanto el m�todo como la direcci�n son obligatorios.', 16, 'VFPRestClient')
				Return
			Endif
			.cleanRequest()
			.Verb 		= tcVerb
	...
	Procedure AddHeader(tcHeader, tcValue)
		If Pcount() != 2
			MessageBox('Faltan par�metros: tanto la clave como el contenido de header son obligatorios.', 16, 'VFPRestClient')
			Return .f.
		Endif
	...
	Procedure addProxy(tcHeader, tcValue)
		If Empty(tcHeader) or Type('tcHeader') != 'C'
			MessageBox('Nombre inv�lido', 16, "VfpRestClient")
			Return .f.
		Endif
	...
	Function Send
		If !This.isConnected()
			MessageBox('En este momento la se�al de internet no est� disponible, int�ntelo m�s tarde.', 16, "VfpRestClient")
			Return .F.
		Endif

		If Empty(This.Verb) or Empty(This.URL)
			MessageBox('Debe ejecutar el m�todo AddRequest() antes de continuar.', 16, "VfpRestClient")
			Return .f.
		Endif
		...
		If !This.isWinHttp And This.getReadyState() <> HTTP_OPEN
			MessageBox('No se pudo inicializar la solicitud [' + this.verb + '] a la direcci�n [' + this.url + ']', 16, "VfpRestClient")
			Return .F.
		Endif
